...
export const fetchReplies = async (bid) => {
  try {
    const response = await axios.get(`https://52.79.234.96:8090/ft/reply/list/${bid}`);
...
export const postReply = async (replyData) => {
  try {
    const response = await axios.post('https://52.79.234.96:8090/ft/reply/insert', replyData);
...
export const updateReply = async (replyData) => {
  try {
    const response = await axios.post('https://52.79.234.96:8090/ft/reply/update', replyData);
    return response.data; // 응답 데이터 반환
  } catch (error) {
...
export const deleteReply = async (rid) => {
  try {
    const response = await axios.post(`https://52.79.234.96:8090/ft/reply/delete/${rid}`);
    return response.data; // 응답 데이터 반환
  } catch (error) {
